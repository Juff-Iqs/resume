...
function loadTheme(theme) {
  ...
  // 🔹 update attribute untuk CSS
  root.setAttribute('color-scheme', theme);

  // 🔹 tambahkan class ke body
  document.body.className = theme;

  // 🔹 ganti semua gambar sesuai theme
...
themeBtn.addEventListener('click', () => {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    // jalankan fungsi tema
    loadTheme(getCurrentTheme());

    // cek kalau layar <= 768px (mobile)
  ...
  langSwitcher.addEventListener("click", () => {
    if (currentLang === "en") {
      ...
      langId.forEach(el => el.style.display = "inline");
      langSwitcher.setAttribute("aria-checked", "id");
      currentLang = "id";
    } else {
      langEn.forEach(el => el.style.display = "inline");
      langId.forEach(el => el.style.display = "none");
      langSwitcher.setAttribute("aria-checked", "en");
      currentLang = "en";
    }
  });
